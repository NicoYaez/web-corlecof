...
function DatosPersonales({ patient }) {
  ...
  return (
    ...
    <div className="flex items-center px-5 py-6 shadow-sm rounded-md bg-white">
      ...
      <div className="mx-5">
        <h4 className="text-2xl font-semibold text-gray-700">Fichas Personal</h4>

      </div >
      {fichaMedica && fichaMedica.paciente ? (
        <div className="mt-2">
          <p className="text-gray-500"><strong>Nombre:</strong> {fichaMedica.paciente.nombre}</p>
          <p className="text-gray-500"><strong>Apellido Paterno:</strong> {fichaMedica.paciente.apellidoPaterno}</p>
          <p className="text-gray-500"><strong>Apellido Materno:</strong> {fichaMedica.paciente.apellidoMaterno}</p>
          <p className="text-gray-500"><strong>Email:</strong> {fichaMedica.paciente.email}</p>
          <p className="text-gray-500"><strong>Edad:</strong> {fichaMedica.edad}</p>
        </div>
      ) : (
